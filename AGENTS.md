...
- **INSERT execution bugs found on the first live VDI attempts (2026-09-11/15), all fixed**: `Escape-SendKeys` in `scripts/ssms-capture.ps1` escaped `(){}[]` in the wrong order (parens/brackets before braces), corrupting long queries — fixed by escaping `{`/`}` first. `to_insert_sql()`'s per-value `-- ColumnName` line comments broke once `ssms-capture.ps1` flattens all whitespace to one line (a line comment with no newline left to stop at swallows everything after it, including the closing `);`) — fixed by switching to `/* ColumnName */` block comments (later superseded by real line breaks, see below). A stale/disconnected query tab (from accumulated tabs across many repeated runs) once caused a "no row found" false failure — worked around by closing extra SSMS tabs before a run. **Confirmed end-to-end working (2026-09-15, Rush Workday, `WD-2609158204TESTCL000EE`)**: INSERT executed, verification SELECT found the row, and every non-date field matched exactly.
  - **SendKeys typing replaced with clipboard paste (2026-09-21)**: after `Correlation_Key`'s default became a much longer hash (~132 chars, see the Correlation_Key entries below), a live run corrupted the query badly — scrambled/dropped characters across many lines, `Msg 102 Incorrect syntax near '}'`. Root cause theory: `SendKeys::SendWait` sent each line as one call with no chunking or per-character pacing; a single very long line (the `Correlation_Key` value) overran the input queue over the Citrix/RDP session, and the resulting desync threw off SSMS's cursor position for every `{ENTER}` afterward, cascading corruption into later lines too. Fixed by pasting the whole query via the clipboard (`Set-Clipboard` + `^v`) instead of typing it. This also made `Escape-SendKeys` (the `(){}[]`-escaping helper, see below) and the `{ESC}`-before-`{ENTER}` IntelliSense workaround unnecessary — both removed.
    - **Corruption recurred once, then root-caused and fixed (2026-09-21)**: same corruption showed up again right after the clipboard-paste change went in, which momentarily looked like the SendKeys-line-length theory above was wrong. Isolated it in two steps, both live on the VDI: (1) `Set-Clipboard`'d the real generated query, pasted into **Notepad** — came through perfectly clean, ruling out clipboard/Citrix-redirection corruption entirely; (2) pasted the same still-set clipboard content **manually into SSMS itself** (`Ctrl+V`, no automation) — also perfectly clean, ruling out an SSMS-side paste-handling bug too. That left only one place the problem could be: the automated script's own timing. `ssms-capture.ps1` was waiting a **fixed 300ms** between `Set-Clipboard` and sending `^v` — not reliably enough for the clipboard to fully propagate over the Citrix/RDP session every time, where a human manually reaching SSMS to paste (step 2 above) took far longer without anyone timing it. Fixed by polling the actual clipboard content (`Get-Clipboard -Raw` compared against the query, up to 10s, 100ms interval) before sending `^v`, instead of guessing at a longer fixed delay — reads-back-to-confirm is both more robust and no slower than necessary.
    - **The very next full live run hit a different, unrelated failure (2026-09-21) — a blocking dialog, not corruption**: the clipboard fix above worked (no more scrambled text), but the INSERT still silently didn't land (`getStagingRow` found zero rows). The INSERT screenshot revealed why: an SSMS tab had `identity-factory/data/latest_insert.sql` open directly as a file (`local_table.py`'s `save_row()` overwrites this file on *every* generation run, for local debugging convenience) — SSMS was showing its "file changed externally, reload?" modal dialog, left over from an earlier interactive session. While that dialog has focus, every keystroke the script sends (`^n`, the paste, `F5`) goes to the dialog instead of the query editor, so nothing actually gets typed or executed — it just silently no-ops, which is why the tab shown in the screenshot was still the old, previously-used one with old content still in it. Same underlying class of issue as the "stale/disconnected query tab" risk noted above, just a new specific trigger. **Fixed**: `ssms-capture.ps1` now sends `{ESC}` immediately after gaining foreground focus, before doing anything else — a safe default response to any such prompt, and a no-op if there's no dialog at all. **Not yet reconfirmed live.**
...
- **Random `Birth_Date`, replacing the old fixed `1998-05-10` (2026-09-18, user-requested)**: `user_payload.py`'s new `random_birth_date()` picks a random date between `1965-01-01` and `2002-12-31` (the 2002 ceiling is the user's explicit request — "at least 2002 and before"; the 1965 floor is this repo's own assumption, not specified, so adjust `BIRTH_DATE_EARLIEST` if a different minimum age is actually wanted). `build_my_rush_jobs_row` now takes an optional `birth_date` param (falls back to `random_birth_date()` when omitted, so the two unrelated legacy callers — `build_copley_attributes`/`build_workflow_input` in `create_test_account.py`/`trigger_workflow.py` — keep working unchanged). **Critically, this must be generated once per identity and shared across every source's row**, not once per row — `generate_identity.py` now does exactly that (one `birth_date` computed before its per-source loop, passed into every `build_my_rush_jobs_row(...)` call), verified live: a `workday`+`copley` identity built with the same `number`+`birth_date` produces matching `Birth_Date`/`User_ID` on both rows.
- **Adding a source to an already-created identity (2026-09-18, user-requested)** — for multi-source test users where some sources were created earlier as separate, single-source runs (e.g. two users created Workday-only, now also need a Copley account): `generate_identity.py` gained `--number` (reuse the 7 digits from an existing Stage_Key instead of allocating a new one — e.g. `5763119` from `WD-955763119ER`) and `--birth-date` (reuse the existing identity's exact `Birth_Date`), both required alongside the existing `--first`/`--last`. `--number` is validated as exactly 7 digits and checked against `used_stage_keys()` for the *new* source+number combination (not the whole identity) before use. Plumbed through `tests/helpers/generatorClient.ts` (`GenerateOptions.number`/`birthDate`) and `create-and-insert-identity.spec.ts` (new hand-edited `NUMBER`/`BIRTH_DATE` constants, alongside `FIRST`/`LAST`).
  - **Handoff JSON merge, not overwrite**: `create-and-insert-identity.spec.ts` writes `pending_aggregation_<firstName>_<lastName>_<lifecycle>.json` keyed by name+lifecycle — running it again for the *same* label but a *different* `SOURCES_TO_CREATE` (the whole point of the add-a-source flow) used to silently overwrite that file with only the new run's row, losing the earlier source's screenshots/attributeRows. Fixed: before writing, it now reads any existing handoff JSON for that label and merges — rows are kept from the existing file unless a new row has the same `stageKey` (re-running the *same* source replaces its own row, doesn't duplicate it). Verified as a logic review (not live, since it needs the VDI), typecheck-clean.
- **Correlation_Key bug found and fixed (2026-09-18)** — found while reviewing the user-provided Rush design doc against a real, manually-correlated multi-source identity (Non-Employee Workforce + Copley Lawson accounts on the same identity cube). That identity's two source accounts had **byte-for-byte identical `Correlation_Key` values** despite different `Stage_Key`s, and the design doc independently confirms Rush's authoritative-source correlation rule matches on Correlation_Key (Full Date of Birth is only a secondary tiebreaker "when multiple matches are found" — not required to differ per source). The generator's old formula, `f"{user_id}-{key}"`, baked each row's own `Stage_Key` into `Correlation_Key`, so a multi-source identity got a *different* `Correlation_Key` per source — they would not have correlated into one identity cube, directly undermining the multi-source feature added earlier this session.
  - **Fixed**: `build_my_rush_jobs_row()`'s default is now `f"{user_id}-{number}"` — since both `user_id` and `number` are already shared across every source in one `generate_identity.py` run (see the Birth_Date fix above for the same "generated/shared once per identity" pattern), this is automatically identical across sources with zero extra plumbing for brand-new multi-source identities. Verified live via the actual CLI: a `workday`+`copley` run produced `Correlation_Key: "617061-3617061"` on both rows.
  - **Add-a-source case needs an explicit override**: an identity created *before* this fix has its Workday-only row's `Correlation_Key` already stored in the DB using the *old* per-row formula (e.g. `"763119-WD-955763119ER"` for `WD-955763119ER`) — the new default can't reproduce that value without knowing it, since it no longer depends on the row's own Stage_Key at all. Added `build_my_rush_jobs_row(..., correlation_key=None)` param, `generate_identity.py --correlation-key`, `generatorClient.ts`'s `GenerateOptions.correlationKey`, and `create-and-insert-identity.spec.ts`'s `CORRELATION_KEY` constant (alongside `NUMBER`/`BIRTH_DATE`) — paste the existing identity's exact stored value (visible in the SailPoint UI account attributes, or a DB SELECT) so the new source's row matches it exactly. **Anyone using the add-a-source flow for an identity created before today needs to supply this** — the two Workday-only users mentioned this session need it when their Copley row is added.
  - **Default made longer/hash-like (2026-09-21, user-requested)**: the short `f"{user_id}-{number}"` default (e.g. `617061-3617061`) looked too small next to real examples (a manually-created identity's Correlation_Key was a ~107-char uppercase hex string). New default: `default_correlation_key(user_id, number)` in `user_payload.py` — `hashlib.sha512(f"{user_id}{number}".encode()).hexdigest().upper()`, a deterministic 128-char uppercase hex string. Still derived only from `user_id`/`number` (both already shared across sources for one identity), so the correctness property from the fix above — identical across every source, different per identity — is unchanged; only the shape got denser. Verified: same identity → identical 128-char value across `workday`/`copley` rows; different identity → different value; explicit `correlation_key` override still bypasses this entirely, unaffected.
  - **Prefixed with `9512` (2026-09-21, user-requested, same day)**: the real reference example (Pooja Vijay's manually-created identity — Non-Employee Workforce + Copley Lawson, see the correlation-bug entry above) has a Correlation_Key that itself starts with `9512`: `9512441111141186CE9CA6FEE37F97534D35DA9435AD2943995227D9D1AB3BB393A35FF419D73C9DA5FF2F21321C6A3BA01FF585124` — the same literal segment `stage_key()` uses. `default_correlation_key()` now returns `f"9512{digest}"` (132 chars total) instead of a bare hash, matching that pattern. Verified via `default_correlation_key('763119', '763119')` → `9512` + 128-char hex, `.startswith('9512')` true.
  - **Extended to exactly 150 chars (2026-09-22, user-requested)**: a second hash segment (`hashlib.sha256(seed + "EXT")`, truncated to 18 chars) is appended after the SHA-512 segment — `4 + 128 + 18 = 150`. Still deterministic from `user_id`/`number` alone, so the identical-across-sources/different-per-identity properties are unchanged. Note: real Rush samples aren't consistent with each other on this — Evelyn Sanders' real RUSH Lawson Correlation_Key turned out to be a bare 128-char hash with **no** `9512` prefix at all, unlike Pooja's NERM/Copley example. This is our own standardized template now, not a replication of any one source's exact real-world convention. Verified: `default_correlation_key(...)` → exactly 150 chars, starts with `9512`.
...
- **Stage_Key format changed (2026-09-16, user-requested)**: `stage_key()` now produces `<prefix>-95<7 random digits>ER` (e.g. `WD-951234567ER`) instead of the old `<prefix>-<10-char yyMMdd+4digit number>TESTCL000<INITIALS>` shape. Applies uniformly to every source (just the prefix changes). `unique_id()` now returns exactly 7 random digits (`random.randint(0, 9999999)`, zero-padded) instead of `yyMMdd` + 4 digits — this value is also still what `User_ID`/`Provider_National_ID`/`Correlation_Key` derive from (`number[-6:]` etc.), so those are unaffected in shape, just sourced from a plain random 7-digit string now. The now-unused `SLOT = "000"` constant and the `time` import were removed. Uniqueness is still enforced the same way (`generate_identity.py`'s `allocate_number()` retries against `excel_store.used_stage_keys()`), not by `stage_key()`/`unique_id()` themselves.
  - **Superseded (2026-09-21, user-requested, all sources)**: `stage_key()` now produces `<prefix>-9512<6 random digits>ER` (e.g. `WD-9512123456ER`) — the `95` literal grew to `9512`, and `unique_id()` dropped from 7 random digits to 6 (`random.randint(0, 999999)`, zero-padded). `user_id = number[-6:]` in `build_my_rush_jobs_row` needed no change — slicing the last 6 of an already-6-digit string just returns the whole string, so `User_ID`/`Provider_National_ID`/`Correlation_Key` are unaffected in length. **Identities created before this change keep their old 7-digit-shape Stage_Key in the DB** — there's no migration, and their number can't be reused with today's `stage_key()` (the `--number`/`NUMBER` add-a-source override path now expects exactly 6 digits, validated in `generate_identity.py`).
...
- **Live-database duplicate check before INSERT (2026-09-21, user-requested)** — `generate_identity.py`'s own collision avoidance (`excel_store.used_stage_keys()`/`used_first_last()`) only checks the local Excel audit log (`identity-factory/data/Copley_test_identities.xlsx`), which is per-machine (Mac and the VDI are separate git checkouts, not guaranteed to be synced) — so it could in theory miss a real duplicate. `create-and-insert-identity.spec.ts` now runs a pre-flight check against the live `My_Rush_Jobs` table, before any SSMS interaction: `findRowsByName()` (new in `tests/helpers/dbClient.ts`, same connection/timeout/normalization pattern as `getStagingRow()`) checks for an existing row with the same `Given_Name`+`Family_Name`, and `getStagingRow()` is reused to check each generated row's `Stage_Key` doesn't already exist. Either match throws a clear error before any SSMS work starts, telling the user to just re-run (which allocates a fresh random name/number). Verified offline: `npx tsc --noEmit` clean. **Not yet exercised live** — the check only ever *should* fire on a genuine collision, which is rare by design (6-digit Stage_Key space, random name pool).
- **`rush` (RUSH Lawson) source template populated (2026-09-22, user-requested)** — third source after Copley/Workday, `source_templates.py`. Built from a real RUSH Lawson account (Evelyn Sanders, found via a live `SELECT ... WHERE Source_Name = 'RUSH Lawson' AND Status = 'Active'` — result turned out inactive, so her account's raw attributes were pulled from her SailPoint **Accounts** tab instead, same "raw underscored view" technique as Pooja/Pablo). Since her real account is inactive, several fields needed explicit user confirmation rather than direct copy:
  - `Relationship_Status`: her real value was `"Inactive"` — user confirmed `"ACTIVE REGULAR"` (matching Copley's convention) for the template instead.
  - `Street_Address`/`City`/`State`/`Postal_Code` (`"628 S Racine Ave 3"`, Chicago, IL): looked like it could be a personal/home address rather than a facility address (unlike Copley's clearly-facility `"2905 STONEHILL RD"`) — user confirmed it's a real work/facility address, safe to reuse.
  - `Username`/`Work_Email` deliberately **not** overridden (left to the scaffold default, same as Copley) — the real sample's `"esanders"` / `"Evelyn_Sanders@rushtst.com"` look like AD-provisioned values synced back into the source post-provisioning, not what a fresh pre-aggregation INSERT should carry.
  - **Not a controllable field, explicitly dropped from scope**: user initially asked for "Account Name"/"Account Number" to both start with `9512XXXXXX` — turned out these map to `SPID`, which isn't one of the 61 `My_Rush_Jobs` columns at all. Comparing real samples (Pooja/NERM: `SPID` = bare `User_ID`; Pablo/Copley: `SPID` = `"20" + User_ID`; Evelyn/RUSH Lawson: `SPID` = `"10" + User_ID`) shows it's computed downstream by SailPoint from `User_ID` with a per-source prefix we don't control — not something this pipeline can set directly. User confirmed: skip it, don't try.
  - Verified via `generate_identity.py --sources rush --lifecycle active`: `Primary_Position` = `"YES"`, `Relationship_Status` = `"ACTIVE REGULAR"`, `Source_Name` = `"RUSH Lawson"` (matching `SOURCES["rush"]["name"]`, not the raw sample's differently-cased `"Rush Lawson"`), `Correlation_Key` 150 chars starting with `9512`. **Not yet live-tested via an actual SSMS INSERT.**
- **SailPoint aggregation trigger via API, opt-in (2026-09-24, user-requested)** — new `tests/helpers/sailpointClient.ts`: minimal SailPoint ISC v3 API client (OAuth2 client-credentials auth, `getSourceIdByName()` looks up any source's GUID by exact display name so nothing is hardcoded, `triggerAggregation()` + `waitForAggregationComplete()` kick off and poll an aggregation task, `aggregateSourceByName()` convenience wrapper). Requires new `.env` vars `SAILPOINT_BASE_URL`/`SAILPOINT_CLIENT_ID`/`SAILPOINT_CLIENT_SECRET` (root `.env`, same file as the DB creds) — user has these or can create them; not present in this repo's tracked files. Uses Node's built-in `fetch` (Node 18+, confirmed Node 24 on this Mac) — no new dependency.
  - Wired into `create-and-insert-identity.spec.ts` behind a new `TRIGGER_AGGREGATION` hand-edited constant (default `false`) — when true and SailPoint is configured (`isSailPointConfigured()`), aggregation is triggered per unique `sourceName` in the identity right after the handoff JSON is written, and awaited before the test finishes. Failures are collected (not thrown immediately) and asserted at the very end, same resilience pattern as the SharePoint-upload and DB-duplicate-check additions earlier — a failure here never costs the already-successful DB verification or the handoff JSON; it just tells the user to fall back to aggregating manually.
  - **Explicitly NOT verified against live SailPoint API docs** — the aggregation-trigger endpoint (`POST /v3/sources/{id}/load-accounts`) and the task-status response shape are both based on recollection of SailPoint's v3 API, flagged clearly in the file's header comment. User explicitly chose to proceed on best-recollection and verify via a live test run rather than pre-confirm the docs first (low stakes — sandbox tenant, wrong path just means a clean 404/error to fix, not silent bad behavior). **`TRIGGER_AGGREGATION` defaults to `false`** specifically because of this — someone needs to deliberately opt in once they've confirmed it actually works, not accidentally rely on unverified behavior.
  - **Not yet live-tested at all** — needs real `SAILPOINT_CLIENT_ID`/`SAILPOINT_CLIENT_SECRET` in `.env` and `TRIGGER_AGGREGATION = true` before a live run can confirm the endpoint path and response shapes are correct.
