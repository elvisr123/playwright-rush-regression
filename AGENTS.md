...
- **INSERT execution bugs found on the first live VDI attempts (2026-09-11/15), all fixed**: `Escape-SendKeys` in `scripts/ssms-capture.ps1` escaped `(){}[]` in the wrong order (parens/brackets before braces), corrupting long queries — fixed by escaping `{`/`}` first. `to_insert_sql()`'s per-value `-- ColumnName` line comments broke once `ssms-capture.ps1` flattens all whitespace to one line (a line comment with no newline left to stop at swallows everything after it, including the closing `);`) — fixed by switching to `/* ColumnName */` block comments (later superseded by real line breaks, see below). A stale/disconnected query tab (from accumulated tabs across many repeated runs) once caused a "no row found" false failure — worked around by closing extra SSMS tabs before a run. **Confirmed end-to-end working (2026-09-15, Rush Workday, `WD-2609158204TESTCL000EE`)**: INSERT executed, verification SELECT found the row, and every non-date field matched exactly.
  - **SendKeys typing replaced with clipboard paste (2026-09-21)**: after `Correlation_Key`'s default became a much longer hash (~132 chars, see the Correlation_Key entries below), a live run corrupted the query badly — scrambled/dropped characters across many lines, `Msg 102 Incorrect syntax near '}'`. Root cause: `SendKeys::SendWait` sent each line as one call with no chunking or per-character pacing; a single very long line (the `Correlation_Key` value) overran the input queue over the Citrix/RDP session, and the resulting desync threw off SSMS's cursor position for every `{ENTER}` afterward, cascading corruption into later lines too — not a timeout/truncation issue, a genuine character-level reliability problem that scales with line length. Fixed by pasting the whole query via the clipboard (`Set-Clipboard` + `^v`) instead of typing it — one atomic OS operation regardless of text length. This also made `Escape-SendKeys` (the `(){}[]`-escaping helper, see below) and the `{ESC}`-before-`{ENTER}` IntelliSense workaround unnecessary — both removed, since paste doesn't go through SendKeys' special-character interpretation or trigger per-keystroke IntelliSense at all. **Not yet reconfirmed live** (the corrupted run was cancelled before commit, so nothing bad landed in the DB) — next live run should be watched to confirm the paste behaves as expected.
